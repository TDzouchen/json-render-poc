...
const isStatePath = (value: unknown): value is string =>
...
const resolveParamValue = (
  ...
  return getByPath(state, value);
};

const resolveParamString = (
  value: unknown,
  state: Record<string, unknown>,
  fallback: string,
): string => {
  const resolvedValue = resolveParamValue(value, state);
  return (
...
export const {
  ...
  components: {
      ...
      return (
        <Collapsible open={open} onOpenChange={setOpen} className="w-full">
          <CollapsibleTrigger asChild>
            <button
              type="button"
              className="flex w-full items-center justify-between rounded-md border border-border px-4 py-2 text-sm font-medium hover:bg-muted transition-colors"
            >
              {props.title}
              <svg
    ...
    Video: ({ props }) => (
      <Media url={props.url} type="video" alt={props.alt ?? undefined} />
    ),
  ...
  actions: {
    ...
    buttonClick: async (params, _setState, state) => {
      const message = resolveParamString(
        params?.message,
        state,
        "Button clicked!",
      );
    ...
    formSubmit: async (params, _setState, state) => {
      const formName = resolveParamString(params?.formName, state, "Form");
...
export const fallbackRenderer = (props: { element: { type: string } }) => (
  <div className="text-xs text-muted-foreground">
    error render | [{props?.element?.type}]
  </div>
);
